...
import type { ContactSummary, Group } from './types.js';
...
function sortPeers(peers: ContactSummary[]): ContactSummary[] {
...
export function groupByLetter(peers: ContactSummary[]): PhonebookSection[] {
	const buckets = new Map<string, ContactSummary[]>();
...
export function groupByGroups(peers: ContactSummary[], groups: Group[]): PhonebookSection[] {
	const sections: PhonebookSection[] = [];
...
export function onlineBucket(peers: ContactSummary[]): ContactSummary[] {
...
function profileHaystack(profile: ContactSummary['profile'] | null | undefined): string[] {
	if (!profile) return [];
	return [
...
export function matchesQuery(peer: ContactSummary, q: string): boolean {
