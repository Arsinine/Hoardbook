...
describe('announce-view', () => {
	...
	it('label shows minutes remaining, ceiling-rounded', () => {
		expect(cooldownLabel(61)).toBe('Ready in 2 min');
		expect(cooldownLabel(120)).toBe('Ready in 2 min');
	});

	it('label floors at 1 min for any sub-minute remainder', () => {
		expect(cooldownLabel(1)).toBe('Ready in 1 min');
		expect(cooldownLabel(59)).toBe('Ready in 1 min');
	});

	it('matches the spec example verbatim (47 min remaining)', () => {
		expect(cooldownLabel(47 * 60)).toBe('Ready in 47 min');
	});
