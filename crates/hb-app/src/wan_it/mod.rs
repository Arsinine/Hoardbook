...
fn usage() -> &'static str {
    "usage: hb-wan-it <serve|probe|canary> [options]\n\
     \n\
     WAN integration harness (M20 W6). Drives the REAL production presence path between live\n\
     endpoints. Not in CI — run as a manual pre-release gate (probe from NAT-A against serve on\n\
     NAT-B and the VPSes) or via the canary daemon (later slice).\n\
     \n\
     ...
     probe  --peer <npub|hbk…> --relay <ws-url>...\n\
            ...
            Runs WAN-P rows P1–P5 against a live serve, TAP 13 out. --flood-relay arms P3\n\
